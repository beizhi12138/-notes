...
# STL常用算法
 ...
 < algorithm > 是STL中头文件最大的一个,范围涉及到比较,交换,查找,遍历,复制,修改等

 < numberic > 体积很小,值包括几个在序列上面进行简单数学运算的模板函数
...
### transform 
 搬运容器到另一个容器中
   ...
   transform(iterator begin,iterator end,iterator biegin2,_func);
     ...
     _func 普通函数或者函数对象(对源容器中的每一个元素进行一些操作,最后将元素返回给目标容器)
...
void test01(){
    ...
    vTarget.resize(v.size());//目标容器一定要预留空间,否则无法存储
...
void test02(){
    ...
    vector<Person>::iterator it=find_if(v.begin(),v.end(),P2());
    if(it != v.end()){
        cout << it->name << it->age <<endl;
    }
}
...
### adjacent_find
  查找相邻重复元素
    ...
    函数原型
      ajacent_find(iterator begin,iterator end);

      返回相邻重复元素的第一个位置的迭代器
```cpp
void test01(){
    vector<int> v;
    v.push_back(0);v.push_back(1);v.push_back(2);v.push_back(2);
...
void test01(){
    vector<int> v;
    v.push_back(1);v.push_back(2);v.push_back(2);v.push_back(1);
...
void test02(){
    vector<Person> v;
    Person p("A",20);Person p2("B",30);Person p3("C",20);
...
### count_if
  条件统计元素个数
     ...
     函数原型
       ...
       count_if(iterator begin,iterator end,pred);
...
void test01(){
    vector<int> v;
    v.push_back(1);v.push_back(2);v.push_back(2);v.push_back(1);
    int result=count_if(v.begin(),v.end(),Greater1()); 
    cout << result << endl;
};
//统计自定义数据类型
class Person{
...
void test02(){
    ...
    Person p("A",20);Person p2("B",30);Person p3("C",20);
    v.push_back(p);v.push_back(p2);v.push_back(p3);
    int result =count_if(v.begin(),v.end(),Age20());
    cout << result<<endl;
}
int main()
{
...
## 常用排序算法
     ...
     random_shuffle  指定范围内的元素进行随机调整次序
...
### sort
     ...
     函数原型  
        ...
        pred是谓词,如果不传谓词那么默认是升序排序，且自定义数据类型必须要传谓词
```cpp
//内置数据类型排序
class Greater1{
    public:
...
void test01(){
    vector<int> v;
    v.push_back(10);v.push_back(30);v.push_back(20);v.push_back(50);v.push_back(66); 
    sort(v.begin(),v.end(),MySort()); //默认是升序，传递函数对象改为降序
...
void test02(){
    vector<Person> v;
    Person p("A",20);Person p2("B",30);Person p3("C",18);
    v.push_back(p);v.push_back(p2);v.push_back(p3);
...
void test01(){
    ...
    cout << "random_shuffle before" <<endl;
    for_each(v.begin(),v.end(),Myprint());
    random_shuffle(v.begin(),v.end());
    cout << "random_shuffle end"<<endl;
    for_each(v.begin(),v.end(),Myprint());
};
...
```
### merge
  将两个容器的元素进行合并，并存储到另一个容器中

  两个容器必须是有序的，才能存储到另一个容器中

     函数原型

     merge(iterator begin1,iterator end1,iterator begin2,iterator end2,iterator vtarget)
...
### copy
  将容器内指定范围的元素拷贝到另一容器中
     ...
     函数原型
       copy(iterator begin,iterator end,iterator TargetBegin);
```cpp
void test01()
{
    vector<int> v;
...
void test01()
{
    ...
    v.push_back(20);v.push_back(66);v.push_back(20);
    replace(v.begin(),v.end(),20,56);
    for_each(v.begin(),v.end(),Myprint());
    cout <<endl;
};
...
## 常用算数生成算法
   ...
   算数生成算法常用于小型算法，使用时包含的头文件为 < numeric >

      accumulate 计算容器所有元素累计总和
...
## 常用集合算法
    ...
    set_difference 两个容器的差集

### set_intersection

   求两个容器的交集

   set_intersection(iterator begin1,iterator end1,iterator begin2,iterator end2,iterator Target);
...
void test01()
{
    ...
    vector<int> vTarget;
    //目标容器需要预留空间
    //但是两个大容器里有小容器，预留空间取小容器
    vTarget.resize(min(v.size(),v2.size()));
    vector<int>::iterator End=set_intersection(v.begin(),v.end(),v2.begin(),v2.end(),vTarget.begin());
    //set_intersection方法返回的是一个迭代器，这个迭代器指向的是最后一个元素的位置
   
    /**
     * 在遍历的时候，不能使用容器的end迭代器，
     *  要使用集合算法返回给的迭代器、
     *  因为我们已经在容器里预留了空间，但是很可能这个空间是多的
     *  所以我们需要使用集合算法返回的迭代器才不会访问到多余的默认元素
    */
...
### set_unioc
  求两个容器的并集
    ...
    set_union(iterator begin,iterator end,iterator begin1,iterator end2,iterator )
...
void test01()
{
    ...
    vector<int> vTarget;
    //目标容器需要预留空间
    //并集就是两个容器合并到一起且不包含重复元素，所以目标容器预留的空间要足够大，能够放的下并集
    vTarget.resize(v.size()+v2.size());
    vector<int>::iterator End=set_union(v.begin(),v.end(),v2.begin(),v2.end(),vTarget.begin());
    for_each(vTarget.begin(),End,Myprint());
    cout<<endl;
};
...
```
### set_difference
 差集，比如有A和B两个容器，A和B的差集就是，A容器在B容器里不重复的元素
     B和A的差集就是，B容器在A容器里不重复的元素。
 求差集，两个集合必须是有序序列。
...
/**
  本节实战demo
    ...
    演讲比赛管理系统
   需求:
    学校举行一场演讲比赛，共有12个人参加，第一轮为淘汰赛，第二轮为决赛。
    每名选手都有对应的编号如1001-1012
    比赛方式，分组比赛，每组6个人
    第一轮分为两个小组，整体按照选手编号进行抽签后顺序演讲
    十个评委分别给每名选手打分，去除最高分和最低分，求得平均分为本轮选手的成绩
    当小组演讲完后，淘汰组内最后的三个选手，前三名晋级，进入下一轮的比赛，第二轮为决赛，前三名胜出
    每轮比赛过后需要显示晋级选手的信息。
   功能
      开始演讲比赛:完成正常比赛，每个阶段给用户一提示
      查看记录:查看之前的结果，每次结果会记录到文件中文件用.csv后缀
      清空记录:清空文件的数据
      退出程序
 */
 //首先需要实现一个mian函数
 #include <iostream>
using namespace std;
#include "./includes/speechManger.h"
#include <ctime>
...
int main()
{
    ...
    while (true)
    {
        ...
        case 0: //退出系统
            ...
            break;
        case 1: //开始比赛
            speechManger.startSpeech();
            break;
        case 2: //查看记录
            speechManger.showLogs();
            break;
        case 3: // 清空文件
            speechManger.clearFile();
            break;
        default:
            system("cls");
            break;
...
class Player{
    private:
      string name;
      double source_Arr[2];// 最多有两轮得分
    public :
        string getName();//获取name
        double* getSourceArr(); //获取分数
        void setName(string name);//设置name
        void setSource(int index,double num);
};
   //源文件
...
double* Player::getSourceArr(){
    return this->source_Arr;
}
void Player::setName(string name){
    this->name=name;
}
void Player::setSource(int index,double num){
    this->source_Arr[index]=num;
}

//然后我们需要一个演讲比赛管理类，来控制整个比赛的流程
  //头文件
  #pragma once
#include <iostream>
using namespace std;
...
public:
    ...
    void createPlayer();  //创建选手
    void showPlayer();    //显示所有的选手
    void startSpeech();   //开始比赛
    void playerDrawing(); //选手抽签
    void game();          //比赛
    void showSource();    //显示晋级选手
    void saveSource();    //保存分数到文件中
    void loadRecord();    //读取文件
    void showLogs();      //显示往届记录
    void clearFile();     //清空记录

    //创建容器用来存放数据
    vector<int> v;                 //所有选手的编号
    vector<int> v1;                //第一场晋级选手的编号
    vector<int> v2;                //第二场晋级选手的编号
    map<int, Player> m;            //编号对应具体的选手类
    map<int, vector<string>> logM; //存放往届记录的容器
    int playCount;                 //比赛次数
...
{
    cout << endl;
    cout << "********** Speech contest management system **********" << endl;
...
void SpeechManger::createPlayer()
{
    ...
    for (int i = 0; i < names.size(); i++)
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            p.setSource(i, 0.0f); //初始化分数
        }
        int bh = i + 1001;
        //将所有的编号存入容器
        this->v.push_back(bh);
        //将编号对应的选手存入map
        this->m.insert(make_pair(bh, p));
    }
};
void SpeechManger::showPlayer()
{
    for (map<int, Player>::iterator it = this->m.begin(); it != this->m.end(); it++)
    {
        cout << (*it).first << " " << (*it).second.getName() << " " << (*it).second.getSourceArr()[0] << (*it).second.getSourceArr()[1] << endl;
    }
};
void SpeechManger::startSpeech()
{
    ...
    //保存分数到文件中
    this->saveSource();
    cout << "This edition of the competition has ended" << endl;
...
void SpeechManger::playerDrawing()
{
    cout << "The << " << this->playCount << " >> round of the speech contest begins" << endl;
    cout << "The draw began" << endl;
    if (this->playCount == 1)
    {
        //第一轮比赛的抽签
        random_shuffle(this->v.begin(), this->v.end()); //利用随机算法
    ...
    {
        //第二轮比赛的抽签
        random_shuffle(this->v1.begin(), this->v1.end()); //利用随机算法
...
void SpeechManger::game()
{
    //创建临时容器，用来保存分组
    multimap<double, int, greater<double>> groupSource;
    int num = 0; // num用来计算小组人数
    cout << "The " << this->playCount << " round begins" << endl;
    vector<int> gamePlayer; //比赛人员的容器
                            //判断是第几轮比赛用哪个容器
    if (this->playCount == 1)
    {
        gamePlayer = this->v; //第一轮的12个人
    }
    else
    {
        gamePlayer = this->v1; //第二个人的6个人
    }
    //遍历选手开始比赛
    for (vector<int>::iterator it = gamePlayer.begin(); it != gamePlayer.end(); it++)
    {
        num++;
        //评委开始打分
        deque<double> d; //创建容器用来保存分数
        for (int i = 0; i < 10; i++)
        {
            double source = (rand() % 401 + 600) / 10.f; //随机带浮点数的分数
            d.push_back(source);
        }
        //进行升序排序
        sort(d.begin(), d.end(), greater<double>());
        //去掉一个最高分，去掉一个最低分，计算平均分
        d.pop_front();
        d.pop_back();
        double avg = accumulate(d.begin(), d.end(), 0.0f) / d.size();
        //   给相应的选手设置平均分
        this->m[*it].setSource(this->playCount - 1, avg);
        groupSource.insert(make_pair(avg, *it));
        if (num % 6 == 0)
        {
            //显示分数
            cout << "Group" << num / 6 << "standings" << endl;
            for (multimap<double, int, greater<double>>::iterator it1 = groupSource.begin();
                 it1 != groupSource.end();
                 it1++)
            {
                cout << "number: " << (*it1).second << " name: " << this->m[(*it1).second].getName()
                     << " source: " << this->m[(*it1).second].getSourceArr()[this->playCount - 1] << endl;
            }
            //取前三名
            int count = 0; //前三名
            for (multimap<double, int, greater<double>>::iterator it1 = groupSource.begin();
                 it1 != groupSource.end() && count < 3;
                 it1++, count++)
            {
                if (this->playCount == 1)
                {
...
void SpeechManger::showSource()
{
    //创建临时容器用于存放选手编号
    vector<int> groupSource;
    cout << "The players who advanced in the " << this->playCount << " round were" << endl;
    if (this->playCount == 1)
    {
        groupSource = v1;
    }
    ...
    for (vector<int>::iterator it = groupSource.begin(); it != groupSource.end(); it++)
    {
        cout << "Player number :" << *it << " name: " << this->m[*it].getName() << "source: " << this->m[*it].getSourceArr()[this->playCount - 1] << endl;
    }
    cout << endl;
    //比赛完清屏幕一下
    system("pause");
...
void SpeechManger::saveSource()
{
    ofstream ofs;
    ofs.open("speech.csv", ios::app | ios::out); // 追加方式写文件，以写文件打开文件
    for (vector<int>::iterator it = this->v2.begin(); it != this->v2.end(); it++)
    {
        ofs << *it << ","
            << this->m[*it].getSourceArr()[1] << ",";
    }
    ofs << endl;
    ofs.close();
...
void SpeechManger::loadRecord()
{
    ...
    //文件存在且有内容
    ifs.putback(ch); //将上面读取的一个字符读取回来
    string data;
    int num = 1; //一行记录就是一届比赛的成绩
    while (ifs >> data)
    {
        vector<string> vData;
        //这里的ifs>>data是每一行的内容，然后要把这个每一行的记录存储一下
        ...
        while (true)
        {
            ...
            start = pos + 1; //从找到,的位置的下一个开始进行继续查找
        }
        this->logM.insert(make_pair(num, vData));
        num++;
    }
    this->FileEmpty = false;
    ifs.close();
}
void SpeechManger::showLogs()
{
    if (this->FileEmpty)
    {
    ...
    else
    {
        ...
        for (map<int, vector<string>>::iterator it = this->logM.begin(); it != this->logM.end(); it++)
        {
            cout << "********************************" << endl;
            cout << "Record of the " << (*it).first << " competition" << endl;
            cout << "First : "
                 << "number: " << (*it).second[0] << "source: " << (*it).second[1] << endl;
            cout << "Second : "
                 << "number: " << (*it).second[2] << "source: " << (*it).second[3] << endl;
            cout << "Third : "
                 << "number: " << (*it).second[4] << "source: " << (*it).second[5] << endl;
            cout << "********************************" << endl;
        }
    }
    system("pause");
...
void SpeechManger::clearFile(){
 cout << "Confirm the emptying" <<endl;
 cout << "1.clear"<<endl;
 cout << "2.back" <<endl;
 int select=0;
 cin>>select;
