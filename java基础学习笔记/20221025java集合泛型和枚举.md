...
```
### TreeSet
 TreeSet同时实现了Set接口和SortedSet,SortedSet接口是Set接口的子接口，可以实现对集合进行自然排序，TreeSet只能对实现了Comparable接口的类对象进行排序,如果TreeSet里存储的是自定义类型的数据那么要自定义比较器，自定义比较器需要实现Comparable接口然后将自定义的比较器传入TreeSet
