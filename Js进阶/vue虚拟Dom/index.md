...
,获取到老节点和新节点遍历的开始下标和开始节点和结束下标和结束节点

1.遍历开始，
2.比较开始节点，开始节点相同(调用patchVnode比较差异)，移动开始节点和开始下标，进行下一个节点的比较.
3.开始节点不相同，比较结束节点，结束节点相同(调用patchVnode比较差异)，移动结束节点和结束下标，进行下一个节点的比较
4.以上两个条件不满足，比较老节点的开始节点和新节点的结束节点是否相同,相同(patchVnode比较差异)，老节点开始节点和下标，新节点结束节点的节点和下标，操作dom树将老节点的开始节点移动到老节点的结束节点后边,，移动老节点的开始节点的节点和下标，新节点的结束节点的节点和下标，进行下一个节点的比较
5,以上三个条件不满足，比较新节点的开始节点和，老节点的结束节点，相同(调用patchVnode)，操作Dom树将老节点的结束节点移动到老节点的开始节点之前,
6.4个条件都不满足，将老节点存储进map里，那新节点的每一个节点在老节点里寻找对应节点，有对应节点且对应节点相同(patchVnode)，没有对应节点创建Dom插入到老节点的开始节点之前，最后移动新节点的开始节点进行下一个节点的查找

7.遍历完成
8.老节点先遍历完，新节点有剩余，则把剩余的节点添加到Dom树中
9.新节点先遍历完，老节点有剩余，则把多余的节点删除